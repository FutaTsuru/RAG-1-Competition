...
def extract_words(text):
    ...
    target_word_match = re.search(target_word_pattern, text)
    if not(keyword_match) or not(related_word_match) or not(important_word_match) or not(target_word_match):
        return "","","",""
    ...
    target_word = target_word_match.group(1).strip().replace(","," ").replace("　"," ")
    return keyword, related_word, important_word, target_word

def main():
    data = {"index": [], "keyword": [], "related_word": [], "important_word": [], "target_word": []}
    ...
    for _, row in tqdm(question_db.iterrows(), total=len(question_db), desc="回答生成"):
        index = row['index']
        query = row['problem']
        keyword, related_word, important_word, target_word = "","","",""
        s = 0
        while keyword == "" or related_word == "" or important_word == "" or target_word == "":
            response = ask_gpt4(query, system_prompt)
            keyword, related_word, important_word, target_word = extract_words(response)
            s += 1
            if s > 1:
